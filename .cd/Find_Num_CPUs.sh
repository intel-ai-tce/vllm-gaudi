...
#export VLLM_FOLDER_PATH="/home/louie/work/vllm"
...
for pair in "${TOK_PAIRS[@]}"; do
  ...
  for NUMCPU in "${CPU_LIST[@]}"; do
    ...
    python3 server/cpu_binding/generate_cpu_binding_from_csv.py \
      --output ./docker-compose.override.yml

    echo "--- Launching benchmark (NUMCPU=$NUMCPU) ---"
    DOCKER_IMAGE="$DOCKER_IMAGE" docker compose \
      --profile benchmark up \
      --abort-on-container-exit

    LOG_SRC="logs/perftest_inp${INPUT_TOK}_out${OUTPUT_TOK}_user${CONCURRENT_REQ}.log"
    LOG_DST="logs/perftest_inp${INPUT_TOK}_out${OUTPUT_TOK}_user${CONCURRENT_REQ}_cpu${NUMCPU}.log"
